...
describe('ProxyImplNodeStreamAPI', () => {
    ...
    it('aborts correctly if the client closes the connection before the target sent the headers', async () => {
        ...
        try {
            ...
            expect(consoleInfo.mock.calls[3][0]).toBe('Request aborted by client: \'http://localhost:22223/foo\'');
        } finally {
            await new Promise((resolve) => server.close(resolve));
        }
    });
    ...
    it('aborts correctly if the client closes the connection before the target sent the body', async () => {
        ...
        try {
            ...
            expect(consoleInfo.mock.calls[3][0]).toBe('Request aborted by client: \'http://localhost:22224/foo\'');
    ...
    it('retries if the target resets the connection', async () => {
        ...
        try {
            const httpProxy = new ProxyImplNodeStreamAPI(2000, false, noopInterceptorHandler, removeAllHeaderFilter, true, null, null, null, false, loggingUtils.dummyLoggerFactory);
    ...
    it('rejects requests if too many are already waiting', async () => {
        ...
        const httpProxy = new ProxyImplNodeStreamAPI(2000, false, noopInterceptorHandler, headerFilter, false, 10, 10, 2, false, loggingUtils.dummyLoggerFactory);
    ...
    it('rejects requests with invalid HTTP protocols', async () => {
        const headerFilter = new HttpHeaderFilter([]);
        const httpProxy = new ProxyImplNodeStreamAPI(2000, false, noopInterceptorHandler, headerFilter, false, 10, 10, null, false, loggingUtils.dummyLoggerFactory);
    ...
    it.skip('forwards WebSocket connections to echo server',  (done) => {
        (async () => {
            ...
            const httpProxy = new ProxyImplNodeStreamAPI(2000, false, noopInterceptorHandler, headerFilter, false, 10, 10, null, true,loggingUtils.dummyLoggerFactory);
    ...
    it('respects WebSocket connection limits',  async () => {
        ...
        const headerFilter = new HttpHeaderFilter([]);
        const httpProxy1 = new ProxyImplNodeStreamAPI(2000, false, noopInterceptorHandler, headerFilter, false, 0, 10, null, false, loggingUtils.dummyLoggerFactory);
        const httpProxy2 = new ProxyImplNodeStreamAPI(2000, false, noopInterceptorHandler, headerFilter, false, 10, 0, null, false, loggingUtils.dummyLoggerFactory);
