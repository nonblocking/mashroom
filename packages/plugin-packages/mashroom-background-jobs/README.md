...
It is possible to add background jobs via service or as a custom plugin.
...
If *node_modules/@mashroom* is configured as a plugin path, add **@mashroom/mashroom-background-jobs** as *dependency*.
...
> [!WARNING]
> A job is started in the main thread and therefore blocks the event loop.
