...
export default class PortalRestProxyController {
    ...
    async forward(message: IncomingMessageWithContext, socket: Socket, head: Buffer): Promise<void> {
        ...
        try {
            ...
            // First wie need to apply the session middleware for this upgrade req, otherwise we have no user context
            ...
            const restProxyDef: MashroomPortalProxyDefinition = portalApp.restProxies[proxyName];
