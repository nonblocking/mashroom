...
import systemJSImportMapConnector from './systemJSImportMapConnector';
...
import type ImportMapManager from './ImportMapManager';
...
export type LoadedPortalAppInternal = {
    ...
    lifecycleHooks: MashroomPortalAppLifecycleHooks | undefined;
...
let _systemJsLoader: Promise<void> | undefined;
...
export default class MashroomPortalAppServiceImpl implements MashroomPortalAppService {
    ...
    constructor(restService: MashroomRestService, private _resourceManager: ResourceManager, private _importMapManager: ImportMapManager) {
    ...
    async loadApp(hostElementId: string, pluginName: string, instanceId: string | undefined | null, position?: number | undefined | null,
        ...
        if (instanceId && this._findLoadedPortalApps(pluginName, instanceId).length > 0) {
            throw new Error(`App ${pluginName}' with instance id ${instanceId} is already loaded!`);
        }

        console.info(`Loading App '${pluginName}' with instance id: ${instanceId || '<undefined>'}`);

        const loadedApp = await this._internalLoadApp(hostElementId, pluginName, instanceId, false, position, overrideAppConfig);

        loadedApp.lifecycleHooks = await this._startApp(loadedApp);
    ...
    async loadAppModal(pluginName: string, title?: string | undefined | null, overrideAppConfig?: any | undefined | null,
        ...
        const loadedApp = await this._internalLoadApp(HOST_ELEMENT_MODAL_OVERLAY, pluginName, null, true, null, overrideAppConfig);
        loadedApp.lifecycleHooks = await this._startApp(loadedApp);
        if (this._watch) {
            this._devModeStartCheckForAppUpdates(loadedApp);
        }
    ...
    async reloadApp(id: string, overrideAppConfig?: any | undefined | null): Promise<MashroomPortalLoadedPortalApp> {
        const loadedAppInternal = this._findLoadedApp(id);
        if (!loadedAppInternal) {
            throw new Error(`Cannot reload App because id not found: ${id}`);
        }
        ...
        try {
            await this._executeWillBeRemovedCallback(loadedAppInternal);
            ...
            let appSetup = await this._internalLoadAppSetup(pageId, loadedAppInternal.pluginName, loadedAppInternal.instanceId);
            ...
            await this._loadResources(loadedAppInternal);
            console.info(`Reloading App '${loadedAppInternal.pluginName}' with setup:`, appSetup);
            loadedAppInternal.lifecycleHooks = await this._startApp(loadedAppInternal);
    ...
    async unloadApp(id: string): Promise<void> {
        const loadedAppInternal = this._findLoadedApp(id);
        if (!loadedAppInternal) {
        ...
        try {
            ...
            await this._unsubscribeFromMessageBus(loadedAppInternal);
            ...
            console.error(`Unloading App '${loadedAppInternal.pluginName}' failed!`, error);
        }
    }
    ...
    async loadAppClientBootstrap(hostElementId: string, pluginName: string): Promise<ClientBootstrapReference> {
        const loadedApp = await this._internalLoadApp(hostElementId, pluginName, undefined, false);
        if (this._watch) {
            this._devModeStartCheckForAppUpdates(loadedApp);
        }

        if (!loadedApp.appSetup || loadedApp.appSetup.pluginMissing) {
    ...
    async checkLoadedPortalAppsUpdated(): Promise<Array<string>> {
        /* if we are not in dev mode _lastUpdatedCheckTs is the page load time */
        try {
            const updatedApps = await this._getUpdatedAppsSince(this._lastUpdatedCheckTs);
    ...
    async prefetchResources(pluginName: string): Promise<void> {
        const pageId = this._getPageId();
        const appSetup = await this._internalLoadAppSetup(pageId, pluginName, null);
        ...
        if (appSetup.resources.js) {
            appSetup.resources.js.forEach((jsResource_2) => this._addPrefetchLink(this._getJSResourceUrl(jsResource_2, appSetup)));
        }
        if (appSetup.sharedResources?.css) {
            appSetup.sharedResources.css.forEach((jsResource_4) => this._addPrefetchLink(this._getSharedCSSResourceUrl(jsResource_4, appSetup)));
        }
        if (appSetup.resources.css) {
            appSetup.resources.css.forEach((jsResource_5) => this._addPrefetchLink(this._getCSSResourceUrl(jsResource_5, appSetup)));
        }
    }
    ...
    private async _internalLoadApp(portalAppAreaId: string, pluginName: string, instanceId: string | undefined | null, modal: boolean,
    ...
    private async _internalLoadAppSetup(pageId: string, pluginName: string, instanceId: string | undefined | null): Promise<MashroomPortalAppSetup> {
        if (instanceId) {
    ...
    private async _loadResources(loadedPortalApp: LoadedPortalAppInternal): Promise<void> {
        const appSetup = loadedPortalApp.appSetup;
        if (!appSetup) {
            throw new Error('appSetup not loaded');
        }
        if (appSetup.pluginMissing) {
            return;
        }
        if (!appSetup.resources.js?.length) {
            throw new Error(`No JS resources found to load for App ${appSetup.pluginName}`);
        }

        loadedPortalApp.loadedModules = [];

        // JavaScript
        // Load the scripts/modules sequentially, first the shared ones
        ...
        if (!appSetup.resources.moduleSystem || appSetup.resources.moduleSystem === 'none') {
            loadJsPromise = appSetup.resources.js.reduce(
                (promise, jsResource) => promise.then(() => this._resourceManager.loadJs(this._getJSResourceUrl(jsResource, appSetup),
                    loadedPortalApp)),
                loadJsPromise);
        } else if (appSetup.resources.moduleSystem === 'ESM') {
            loadJsPromise = appSetup.resources.js.reduce(
                (promise, jsResource) => promise.then(async () => {
                    const module = await import(/* webpackIgnore: true */ this._getJSResourceUrl(jsResource, appSetup));
                    loadedPortalApp.loadedModules?.push(module);
                }),
                loadJsPromise);
        } else if (appSetup.resources.moduleSystem === 'SystemJS') {
            await this._ensureSystemJS();
            this._importMapManager.addImportMap(appSetup, systemJSImportMapConnector);
            loadJsPromise = appSetup.resources.js.reduce(
                (promise, jsResource) => promise.then(async () => {
                    const module = await System.import(this._getJSResourceUrl(jsResource, appSetup));
                    loadedPortalApp.loadedModules?.push(module);
                }),
                loadJsPromise);
        }
    ...
    private async _ensureSystemJS() {
        if (typeof System !== 'undefined') {
            return;
        }

        if (_systemJsLoader) {
            return _systemJsLoader;
        }

        console.info('Loading SystemJS');
        const systemJsUrl = this._apiPath.replace('/api', `/${SYSTEMJS_JS_FILE}`);
        _systemJsLoader = this._resourceManager.loadJs(systemJsUrl);
        return _systemJsLoader;
    };

    private async _startApp(loadedApp: LoadedPortalAppInternal): Promise<MashroomPortalAppLifecycleHooks | undefined> {
        const {id, portalAppHostElement, pluginName, appSetup} = loadedApp;
        ...
        if (bootstrapRetVal) {
            if ('then' in bootstrapRetVal && typeof (bootstrapRetVal.then) === 'function') {
                ...
                try {
                    ...
                    return lifecycleMethods ?? undefined;
                } catch (error: any) {
                    handleError(error);
                    throw error;
                }
                ...
                console.info(`App successfully loaded: ${pluginName}`);
                return bootstrapRetVal as MashroomPortalAppLifecycleHooks;
            }
        } else {
            console.info(`App successfully loaded: ${pluginName}`);
        }
    ...
    private _determineClientBootstrap(loadedPortalApp: LoadedPortalAppInternal): MashroomPortalAppPluginBootstrapFunction {
        const appSetup = loadedPortalApp.appSetup!;
        let bootstrap: MashroomPortalAppPluginBootstrapFunction | undefined;

        if (loadedPortalApp.loadedModules.length) {
            bootstrap = loadedPortalApp.loadedModules.find((m) => appSetup.clientBootstrapName in m)?.[appSetup.clientBootstrapName]
                || loadedPortalApp.loadedModules.find((m) => 'default' in m && appSetup.clientBootstrapName in m.default)?.default?.[appSetup.clientBootstrapName];
        }

        if (!bootstrap) {
            // Check if the launch function name in appSetup would cause a conflict
            const conflictingApp = loadedPortalAppsInternal.find((la) => la.globalBootstrap
                && la.appSetup?.clientBootstrapName === appSetup.clientBootstrapName
                && la.pluginName !== loadedPortalApp.pluginName);
            if (conflictingApp) {
                throw new Error(`The global bootstrap function name '${appSetup.clientBootstrapName}' conflicts with an already loaded App: ${conflictingApp.pluginName}`);
            }
            bootstrap = (global as any)[appSetup.clientBootstrapName];
        }

        if (!bootstrap) {
            throw new Error(`App bootstrap function not found: ${appSetup.clientBootstrapName}`);
        }
        ...
        if (!_modalInitialized) {
            ...
            const closeEventListener = async () => {
                ...
                if (currentApp) {
                    ...
                    await this.unloadApp(currentApp.loadedApp.id);
                    currentApp.onClose?.();
                }
            };
    ...
    private _appendAppWrapper(id: string, pluginName: string, title: string | undefined | null, portalAppAreaId: string, position?: number | undefined | null) {
        console.debug(`Adding wrapper for app: ${pluginName}, host element: ${portalAppAreaId}, position: ${String(position)}`);
    ...
    private async _unsubscribeFromMessageBus(loadedAppInternal: LoadedPortalAppInternal) {
        const clientServices: MashroomPortalClientServices = (global as any)[WINDOW_VAR_PORTAL_SERVICES];
        const mmb = clientServices.messageBus as MashroomPortalMasterMessageBus;
        await mmb.unsubscribeEverythingFromApp(loadedAppInternal.id);
    }
    ...
    private async _executeWillBeRemovedCallback(loadedAppInternal: LoadedPortalAppInternal): Promise<void> {
        const handleError = (error: Error) => {
    ...
    private _addPrefetchLink(url: string) {
    ...
    private async _devModeCheckForAppUpdates() {
        try {
            const updatedApps = await this._getUpdatedAppsSince(this._lastUpdatedCheckTs);
            ...
            if (Array.isArray(updatedApps) && updatedApps.length > 0) {
                ...
                // Filter and update apps sequentially
                for (const app of updatedApps.filter(app =>
                    this._watchedApps.find(watchedApp => watchedApp.pluginName === app.name)
                )) {
                    try {
                        await this._updateApp(app);
                    } catch (error) {
                        console.warn('Failed to update app:', app.name, error);
                    }
                }
            }
        } catch (error) {
    ...
    private _devModeStartCheckForAppUpdates(loadedAppInternal: LoadedPortalAppInternal) {
        // Remove existing
        this._devModeStopCheckForAppUpdates(loadedAppInternal);
    ...
    private _devModeStopCheckForAppUpdates(loadedAppInternal: LoadedPortalAppInternal) {
