...
import type {PortalAppManagementService, DataLoadingService} from '../../../type-definitions';
...
export default class AddAppDropdownMenu extends PureComponent<Props, State> {
    ...
    onAppDragStart(event: DragEvent, name: string) {
        this.props.portalAppManagementService.prepareDrag(event, null, name);
        ...
        return (
            <DropdownMenu className='add-app-dropdown-menu' labelId='addApp' onOpen={this.onOpen.bind(this)} closeRef={this.onCloseRef.bind(this)}>
                <div className='add-app-content'>
                    <input type='search' placeholder={filterLabel} value={this.state.filter || ''} onChange={(event) => this.onFilterChange(event.target.value)} ref={(e) => this.inputElem = e}/>
                    <div className='app-list'>
                        <AvailableAppsPanelContainer onDragStart={this.onAppDragStart.bind(this)} onDragEnd={this.onAppDragEnd.bind(this)} filter={this.state.filter}/>
                    </div>
                </div>
            </DropdownMenu>
        );
